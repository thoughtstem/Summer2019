...
@section{Numbers}
@bold{Example 1}
Adds all odd numbers from 1 to 51 
@codeblock{
#lang racket
(apply +
  (map (curry + -1)
...
from the three lists that indicate width, length, and colour of the file-icon.
...
@bold{Example 9}
Make a list of circles that alternate colors of the rainbow.
@codeblock{
 #lang racket
 (require pict)
...
list through the function 'lambda,' which colorizes image x with the inputed color. (rainbow (circle 50)) returns a list of circles
...
Inside the function rainbow, we map the function lambda ('lambda' is used to define an 'anonymous'/'temporary' function) to
a list of colors. This means we will generate a list of images that are created by running every element in the color
list through the function 'lambda,' which colorizes image x with the inputed color.
